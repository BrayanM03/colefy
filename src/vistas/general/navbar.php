...
<script>
	...
	document.addEventListener('DOMContentLoaded', function() {
        ...
        // 1. Cambiamos el tema visual y el estado del switch
        document.body.classList.toggle('dark', tema === 'dark');
        switchEl.checked = (tema === 'dark');

        // 2. Seleccionamos el CONTENEDOR del ícono (el hermano del switch)
        const contenedorIcono = switchEl.nextElementSibling;

        if (contenedorIcono) {
            // 3. Recreamos la etiqueta <i> desde cero con el ícono correcto
            const iconoNombre = tema === 'dark' ? 'sun' : 'moon';
            contenedorIcono.innerHTML = `<i data-feather="${iconoNombre}"></i>`;
            
            // 4. Le decimos a Feather que dibuje el nuevo SVG
