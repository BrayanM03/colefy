...
<svg aria-labelledby="sinpermiso" alt="Sin permiso" class="sin-permiso" viewBox="0 0 600 300">
  <title id="svgtitle1">Sin permiso</title>
  <style>
    .texto-base {
      font-family: Arial, sans-serif;
      font-weight: bold;
      font-size: 70px; /* Ajustado para el tamaño */
    }
  </style>

  <g opacity=".5" fill="#3B3D3D">
    <text x="180" y="130" class="texto-base">Sin</text>
    <text x="180" y="210" class="texto-base">permiso</text>
  </g>

  <g fill="#FFF">
    <text x="175" y="125" class="texto-base">Sin</text>
    <text x="175" y="205" class="texto-base">permiso</text>
  </g>
</svg>
<h1 id="mensaje-principal">Error no tienes los permisos para acceder, seras redirigido en <span id="segundos-restantes">5</span> segundos.</h1>
...
<script>
        ...
        const mensajePrincipal = document.getElementById('mensaje-principal').textContent;
        const segundosSpan = document.getElementById('segundos-restantes');
        
        
        // 3. Lógica del contador y redirección
        let segundos = 4; // Inicia el contador en 5
        
        function actualizarContador() {
            // Actualiza el texto en el span dentro del H1 principal
            segundosSpan.textContent = segundos;
            
            if (segundos > 0) {
                // Si aún quedan segundos, decrementa el contador
                segundos--;
                setTimeout(actualizarContador, 1000); // Llama a la función de nuevo después de 1 segundo (1000ms)
            } else {
                // Cuando el contador llega a 0, se realiza la redirección
                window.location.href = "../../../index.php"; // <-- ¡CAMBIA ESTA URL a donde quieras redirigir!
            }
        }
