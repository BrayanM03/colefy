function cargarCuadriculaSemanal() {
    ...
    const fechasSemana = [];
    const nombresDias = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo'];
    ...
    fetch(`${BASE_URL}api/asistencias.php?tipo=obtener_semanal&id_grupo=${idGrupo}&fecha_inicio=${fechaInicio}&fecha_fin=${fechaFin}`)
        .then(res => res.json())
        .then(res => {
            if (!res.estatus) {
