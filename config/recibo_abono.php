...
if(!$recibo_resp['estatus']){
    ...
    header("Location: " . BASE_URL . "sin_permiso/$mensaje");
...
class PDF extends FPDF
{
    ...
    function DibujarRecibo($y_inicio, $datos_empresa, $productos, $suma_pagos, $total, $saldo_pendiente, $abonado, $pago_el, $es_cliente, $estatus = 1)
    {
        ...
        $this->Image('./static/img/escuelas/' . $datos_empresa['logo'], 10 + 7, $y_inicio + 6, $ancho_logo - 10, $alto_logo - 10);
       ...
       /*  print_r($suma_pagos);
        ...
        if ($es_cliente) {
            ...
            $this->SetX(60);
            $this->Cell(85, 5, $fn_iconv($datos_empresa['usuario']['nombre'] .' '. $datos_empresa['usuario']['apellido']/* 'CP Gabriela Ruiz' */), 0, 1, 'C');
        }
    }
    ...
    function Watermark($y_base, $estatus = 1) 
    {
       // 1. Guardar la posición actual X, Y
            ...
            $escuela_data = $GLOBALS['datos_empresa'];
            $logo_path = './static/img/escuelas/opacos/' . $escuela_data['logo'];
            $ancho_recibo = 195;
